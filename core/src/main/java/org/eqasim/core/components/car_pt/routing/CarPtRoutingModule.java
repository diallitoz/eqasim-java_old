...
public class CarPtRoutingModule implements RoutingModule {
	 ...
	 * toFacility, double departureTime, Person person) { //Id<AVOperator>
	 * operatorId = choiceStrategy.chooseRandomOperator(); //return
	 * calcRoute(fromFacility, toFacility, departureTime, person, operatorId);
	 * 
	 * Leg leg = PopulationUtils.createLeg("car_pt"); leg.setTravelTime(600.0);
	 * 
	 * Route route = new GenericRouteImpl(fromFacility.getLinkId(),
	 * toFacility.getLinkId()); route.setTravelTime(600.0);
	...
	public List<? extends PlanElement> calcRoute(Facility fromFacility, Facility toFacility, double departureTime,
		...
		Coord prCoord = new Coord(X, Y); // XY coords of some "test" P&R station somewhere in your scenario. Later
		...
		List<? extends PlanElement> carElements = carRoutingModule.calcRoute(fromFacility, prFacility, departureTime,
				null);
		...
		List<? extends PlanElement> ptElements = ptRoutingModule.calcRoute(prFacility, toFacility, ptDepartureTime,
				person);
